...
import { User, Building2, Bell, Lock, FileText, HelpCircle, Headphones } from 'lucide-react';
...
function SidebarLayout({
  ...
  return (
    <div className="flex gap-5 p-5 h-[calc(100vh-64px)]">
      ...
      <aside className="w-64 bg-white rounded-[14px] border border-slate-200 shadow-[0_1px_3px_rgba(0,0,0,0.05)] flex-shrink-0 py-3 px-3 h-full overflow-hidden">
...
function TabsLayout({
  ...
  return (
    <div className="flex flex-col min-h-full">
      {/* Tab bar */}
      <div className="bg-white border-b border-slate-200 px-7">
        <div className="flex items-end gap-0 overflow-x-auto">
          {NAV_ITEMS.map((item) => {
            ...
            return (
              ...
              >
                <span className={cn(isActive ? 'text-primary' : 'text-muted-foreground/70')}>{item.icon}</span>
                {item.label}
              </button>
            );
          })}
...
export function Settings(): React.JSX.Element {
  ...
  const layout = searchParams.get('layout') === 'tabs' ? 'tabs' : 'sidebar';
      ...
      >
        ...
        <div className="flex-1 overflow-hidden">
          {layout === 'sidebar' ? (
            ...
            <TabsLayout active={activeSection} onSelect={setActiveSection} />
          )}
        </div>
      </div>
    </div>
